...
Amtrak<-read.csv("E:\\R-folder\\02-CSV files\\Amtrak.csv")  # read the Amtrack data 
...
Add_sea_Quad_model<-lm(Ridership~t+t_square+Jan+Feb+Mar+Apr+May+Jun+Jul+Aug+
                         Sep+Oct+Nov,data=train) 
...
table_rmse<-data.frame('Model'=c("rmse_linear","rmse_expo","rmse_Quad","rmse_sea_add",
            ...
            'RMSE'=c(rmse_linear,rmse_expo,rmse_Quad,rmse_sea_add,rmse_Add_sea_Quad,rmse_multi_sea,rmse_multi_add_sea))
...
#Combine the predict model and error model data
#Consider this to be best model
