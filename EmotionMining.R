...
txt = readLines("E:\\R-folder\\CSV files\\amazon nokia lumia reviews.txt")
...
example<-get_sentences(txt) ## split all the reviews
...
windows() 
plot(sentiment_vector,type='l',
     main='Plot trajectory',xlab='Narative time',ylab='Emotional valence') 
...
##get_transformed_values function as shown below. 

ft_values <- get_transformed_values(   sentiment_vector,    
            low_pass_size = 3,    x_reverse_len = 100,   padding_factor = 2,  
            scale_vals = TRUE,   scale_range = FALSE )  

plot( ft_values,type ="l",
...
#x-axis represents the passage of time from the beginning to the end of 
##the text, and the y-axis measures the degrees of positive and negative 
##sentiment 

s_v<-get_sentences(my_example_text) 
s_v_sentiment<-get_sentiment(s_v)

plot(   s_v_sentiment,    type="l", 
